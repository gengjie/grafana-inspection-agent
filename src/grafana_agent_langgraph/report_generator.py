...
class ReportGenerator:
    ...
    def format_daily_report(
        ...
        alert_summary: str,
        slow_query_summary: str | None = None,
...
{'─' * 80}
III. Slow Query SQL Diagnosis
{'─' * 80}

{slow_query_summary or 'Slow-query SQL diagnosis not available.'}
{long_term_block}
...
{slow_query_summary or '慢查询 SQL 诊断摘要不可用。'}
{long_term_block}
...
"""
    ...
    @staticmethod
    def format_slow_query_report_for_email(
        slow_query_report: str,
        inspection_time: str | None = None,
        language: str = "zh",
    ) -> tuple[str, str]:
        """Format slow-query SQL diagnostic report for email sending."""
        if language == "en":
            inspection_date = datetime.now(tzutc()).strftime("%B %d, %Y")
            subject = f"Slow Query SQL Diagnostic Report - {inspection_date}"
            title = "Slow Query SQL Diagnostic Report"
            gen_label = "Generated Time"
            gen_time = inspection_time or datetime.now(tzutc()).strftime("%Y-%m-%d %H:%M:%S UTC")
            footer = "This report is automatically generated by Grafana Inspection Agent"
            empty_text = "No slow-query report content"
        else:
            inspection_date = datetime.now(tzutc()).strftime("%Y年%m月%d日")
            subject = f"慢查询 SQL 诊断报告 - {inspection_date}"
            title = "慢查询 SQL 诊断报告"
            gen_label = "生成时间"
            gen_time = inspection_time or datetime.now(tzutc()).strftime("%Y-%m-%d %H:%M:%S UTC")
            footer = "本报告由Grafana Inspection Agent自动生成"
            empty_text = "暂无慢查询报告内容"

        cleaned_report = ReportGenerator._strip_yaml_front_matter(slow_query_report)
